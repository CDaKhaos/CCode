...
static ClassFactory<TiXmlElement, 10000> xmlElementFactory_;
static ClassFactory<TiXmlComment, 1000> xmlCommentFactory_;
static ClassFactory<TiXmlText, 1000> xmlTextFactory_;
static ClassFactory<TiXmlDeclaration, 1000> xmlDeclarationFactory_;
static ClassFactory<TiXmlUnknown, 1000> xmlUnknownFactory_;
static ClassFactory<TiXmlDocument, 1000> xmlDocumentFactory_;
...
static ClassFactory<TiXmlAttribute, 10> xmlAttributeFactory_;
...
void TiXmlBase::PutString( const TIXML_STRING& str, TIXML_STRING* outString )
{
	...
	while( i<(int)str.length() )
	{
		...
		else if ( c < 32 || c > 126 )
		{
			...
			char buf[ 32 ];
			cc_sprintf( buf, "&#x%02X;", (unsigned) ( c & 0xff ) );
...
TiXmlBase::StringToBuffer::StringToBuffer( const TIXML_STRING& str )
{
	...
	if ( buffer )
	{
		cc_strcpy( buffer, str.c_str(), iSize );
	}
}
...
{
	if (cc_sscanf( value.c_str(), "%d", ival ) == 1 )
...
{
	if (cc_sscanf( value.c_str(), "%lf", dval ) == 1 )
		return TIXML_SUCCESS;
	return TIXML_WRONG_TYPE;
}
